...
\begin{itemize}		
	\item[$\alph$] translate it into an NFA
		\answer{\centeredimg{rl_1_a}}

	\item[$\alph$] remove (semantic preserving) the $ \epsilon $ transitions
		\answer{\centeredimg{rl_1_b}}

	\item[$\alph$] create the corresponding DFA
...
\question{2. Consider the following language: \\ 
	A word is in the language if and only if it contains an even number
	of $A$s an odd number of $B$s and precisely one $C$.
  Design a DFA for this language. Explain your design  (what information do you ``store'' in your states?)}
